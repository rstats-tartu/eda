...
incidence_filt <- filter(incidence, !summary_var(Site)) %>% 
  select(-label, -source)
```

Data is **`r tolower(label)`** from `r data_source`. 


Table is still in the wide format, let's convert table into long/tidy format and filter out missing data.
```{r}
