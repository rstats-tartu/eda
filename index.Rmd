...
## To save typing, we create function to calculate incidence on grouped table
calculate_incidence <- function(mytbl){
  summarise(mytbl, cases = sum(cases),
            ave_pop = sum(ave_pop)) %>% 
    mutate(incidence = cases/(ave_pop/1E5))
}
```
...
**Men get more cancer in Estonia.** Clearly, increasing trend for new cancer cases is still visible after normalisation for population size. 
...
total_incidence_year_sex <- incidence_year_sex %>% 
  filter(Site == "All malignant neoplasms (C00-C97)")
## we give new data to plot and we also must supply aes for Sex
p %+% total_incidence_year_sex + 
  aes(y = incidence, color = Sex) +
...
incidence_range <- total_incidence_year_sex %>% 
  ...
  summarise(Min = round(min(incidence)),
            ...
            increase = sprintf("%1.0f%%", (Max-Min)*100/Min))
knitr::kable(incidence_range, col.names = c("Sex", "2000", "2014", "Increase"))
```

We can see that compared to counts both men and women show even bigger increase in cancer rates in Estonia. 
...
```{r}
incidence <- filter(incidence, !str_detect(Site, "All malignant"))
incidence_year_site_sex <- incidence %>% 
