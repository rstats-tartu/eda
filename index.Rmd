...
```{r}
## trim characters until first [[:alpha:]], we keep also original `Specified site` values in Site_old, rename `Specified site` to Site and remove constant columns from table
cases <- mutate(cases, 
                    Site_old = `Specified site`,
...
**How big is the increase per year**, although we have count data and perhaps should use Poisson model. 
However, as the mean gets further away from zero even as low as 10, the Poisson distribution looks like a normal distribution. 
It becomes symmetric with it a mode at the mean.
So we can safely fit normal linear model, parameter estimates will be similar to Poisson.

```{r}
mod1 <- lm(total_cases ~ Year, total_cases_per_year)
## tidy is from broom package
tidy(mod1)
```

We can see that number of new cancer cases has increased by `r round(tidy(mod1)$estimate[2])` each year during this period. 
NB! intercept term does not make sense in this case.


So we have seen `r round(tidy(mod1)$estimate[2])` more new cases each year during 2000 and 2014. 
